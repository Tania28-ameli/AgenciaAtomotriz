...
namespace Manejador
{
    public class ManejadorRefacciones
    {
        ...
        public string GuardarRefacciones(TextBox codigoBarras, TextBox Nombre, TextBox descripcion, TextBox Marca)
        {
            try
            {
                return b.Comando(($"insert into refacciones values (null'{codigoBarras.Text}','{Nombre.Text}', '{descripcion.Text}', '{Marca.Text}')"));
            }
            catch (Exception)
            {
        ...
        {
            Tabla.Columns.Clear();
            DataTable datos = b.Consultar($"SELECT * FROM Refacciones WHERE Tipo LIKE '%{filtro}%' OR CodigoHerramienta LIKE '%{filtro}%' AND Nombre LIKE '%{filtro}%'", "Herramientas").Tables[0];
